...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  Lcd_Init();
  Lcd_Send_String("Bekleniyor...");

  HAL_UART_Receive_IT(&huart2, &tek_harf, 1);
...
void message(){
	  if (flag=1){
		  flag=0;
	    	// 1. Eğer gelen karakter '?' ise ekranı temizle
	        if (tek_harf == '?')
	        {
	            Lcd_Clear();
	            Lcd_Cursor(0, 0);
	        ...
	        {
	            // 2. İlk mesaj geldiyse ekranı temizleyip yazmaya başla
	            static int ilk_mesaj = 1;
	            if(ilk_mesaj) {
	                Lcd_Clear();
	                ilk_mesaj = 0;
	            }
	            Lcd_Data(tek_harf); // Harfi ekrana bas
	        }
	  }
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    if(huart->Instance == USART2)
    {
    	...
    	// Gürültüden kaynaklı hataları temizle (Hayati önem taşır)
    	        __HAL_UART_CLEAR_OREFLAG(huart);
       flag=1;

        // 3. KRİTİK ADIM: Bir sonraki harfi beklemek için kesmeyi tekrar kur
        HAL_UART_Receive_IT(&huart2, &tek_harf, 1);
